...
if __name__ == '__main__':
    class EffPosNode(object):
        ...
        def foot_contact_cb(self,msg):
            if self.Contact == 0 and len(msg.states)>4:
                if self.Reflex == 1:
                    self.CON.set_ref({'piston_axis':{'pos':self.piston_limit[1],'eff':0,'k_eff':0.0}})

                self.Contact = 1
                print "Touch down - Contacts: ",len(msg.states)
            if self.Contact == 1 and len(msg.states) == 0:
                if self.Reflex == 1:
                    self.CON.set_ref({'piston_axis':{'pos':self.piston_limit[0],'eff':0,'k_eff':0.0}})

                self.Contact = 0
                print "Lift off"

        def pid_param_update(self):
            self.CON._joints['spring_axis'].set_params({'P': 50.0, 'I': 0.0, 'D': 0.0})
            self.CON._joints['piston_axis'].set_params({'P': 200.0, 'I': 0.0, 'D': 15.0})
        ...
        def Initialize(self):
            ...
            for i in range(100):
                self.CON.set_ref({'spring_axis':{'pos':0.05+i/100.0*0.45,'eff':0,'k_eff':0.0}})
                rospy.sleep(0.01)
    ...
    eff_pos_node.Go=0
    eff_pos_node.Contact = 1
    eff_pos_node.piston_limit=[0.02,0.23]
    rospy.Subscriber("/odh/joint_states", JointState, eff_pos_node.on_state_update)
